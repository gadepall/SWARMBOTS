...
\end{equation}
where the various parameters are listed in Table \ref{tab:symbols}.
...
\item {\em GUI-Based Task Assignment:}
...
\item {\em Task Distribution Strategy:}
...
\begin{itemize}
    \item {\em Round-robin assignment:} Tasks are distributed sequentially across all available slave UGVs, regardless of estimated time.
    
    \item {\em Time-aware distance-based allocation:} For each task point, the slave UGV with the lowest estimated arrival time (based on precomputed $t$) is assigned that task. This reduces overall execution time and idle waiting.
\end{itemize}
